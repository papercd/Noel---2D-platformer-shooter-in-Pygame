...
"""
                                                                    #min #max 
sliders.append(Slider(11, 106+21, 150, 10, 0.25, 15))  #size        0.25 ~ 15
sliders.append(Slider(11, 150+21, 150, 10, 1, 10))      #density     1, 10
sliders.append(Slider(11, 194+21, 150, 10, 0, 2))       #rise           0, 2
sliders.append(Slider(11, 238+21, 150, 10, 0, 3))       #spread         0, 3
sliders.append(Slider(11, 282+21, 150, 10, -2, 2))      #wind             -2, 2
"""
...
while running:
    ...
    if not ms:
        ...
        for d in range(0, int(dist), 10):
            ...
            _y = my+sin(a)*d
            if mp:
                for _ in range(round(density)): particles.append(Particle(_x//res, _y//res))
        else:
            if mp:
                for _ in range(round(density)): particles.append(Particle(mx//res, my//res))
    ...
    for p in particles:
        ...
        p.y -= rise
        p.x += ((p.sin * sin(j/(p.sinr)))/2)*spread + wind
